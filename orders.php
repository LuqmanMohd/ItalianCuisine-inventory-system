...
<head>
  ...
  <title>Italian Cuisine : Orders</title>
  <!-- Bootstrap -->
...
<div class="container-fluid">
  ...
  <div class="row">
    ...
    <form action="orders.php" method="post" class="form-horizontal">
      ...
      <div class="form-group">
            ...
            <div class="col-sm-9">
      <input name="oid" type="text" readonly value="<?php if(isset($_GET['edit'])) echo $editrow['fld_order_num']; ?>" class="form-control"> 
      </div>
          </div>
       ...
       <div class="form-group">
            ...
            <div class="col-sm-9">
      <input name="orderdate" type="text" readonly value="<?php if(isset($_GET['edit'])) echo $editrow['fld_order_date']; ?>"class="form-control"> 
      </div>
          </div>
      ...
      <select name="sid" class="form-control" id="staff" required>
        ...
        foreach($result as $staffrow) {
          ?>
          <?php if((isset($_GET['edit'])) && ($editrow['fld_staff_num']==$staffrow['FLD_STF_ID'])) { ?>
            <option value="<?php echo $staffrow['FLD_STF_ID']; ?>" selected><?php echo $staffrow['FLD_STF_NAME'];?></option>
          <?php } else { ?>
            <option value="<?php echo $staffrow['FLD_STF_ID']; ?>"><?php echo $staffrow['FLD_STF_NAME'];?></option>
          <?php } ?>
          <?php
      } // while
      $conn = null;
  ...
  <div class="row">
    ...
    try {
      $conn = new PDO("mysql:host=$servername;dbname=$dbname", $username, $password);
      $conn->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
      $sql = "SELECT * FROM tbl_orders_a175128, tbl_staffs_a175128_pt2, tbl_customers_a175128_pt2 WHERE ";
      $sql = $sql."tbl_orders_a175128.fld_staff_num = tbl_staffs_a175128_pt2.FLD_STF_ID and ";
      $sql = $sql."tbl_orders_a175128.fld_customer_num = tbl_customers_a175128_pt2.FLD_CUST_ID LIMIT $start_from, $per_page";
    ...
    foreach($result as $orderrow) {
      ...
      <tr>
        ...
        <td><?php echo $orderrow['FLD_STF_NAME']; ?></td>
    ...
    <div class="col-xs-12 col-sm-10 col-sm-offset-1 col-md-8 col-md-offset-2">
      <nav>
          ...
          try {
            $conn = new PDO("mysql:host=$servername;dbname=$dbname", $username, $password);
            $conn->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
            $stmt = $conn->prepare("SELECT * FROM tbl_orders_a175128");
            $stmt->execute();
            $result = $stmt->fetchAll();
